...
class Handler:
    ...
    def get_response(self, chat_id, text):
        if self.get_or_create_state(chat_id) == 'start_state' and text == '/start':
            self.storage[chat_id]['state'].start_dialog()
            ...
            if text in ['большую', 'маленькую']:
                ...
                self.storage[chat_id]['state'].select_size()
            ...
            if text in ['наличкой', 'безналичкой']:
                ...
                self.storage[chat_id]['state'].select_payment_form()
            ...
            if text == 'да':
                self.storage[chat_id]['state'].fix_order()
